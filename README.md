<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8"><!--It is bapp of ASCII values-->
    <meta name="viewport" content="width=device-width", intal.scale =0.1>\
    <title>Resume</title>
    <style>
        table,tr,th,td{
            border:1px solid black ;
            border-collapse: collapse;
            padding: 30px;
        }
    </style>
</head>
<body>
       <h1>Alexa</h1><!--Try to write only in whole program one time <h1>-->
       <div>
        <img src="data:image/jpeg;base64," alt="image">
        <figcaption>i am exprienced softare enginner and able to work in a<br>
        team with problem solving skills in creating and designing a software</figcaption>
    </div>
    <hr>
    <div>
        <h2>Contact</h2>
        <span><strong>Email id:</strong>xyz@ggamil.com</span>
        <br>
        <span><strong> Mobile no.</strong>123456789</span>
    </div>
    <hr>
    <div>
        <h2>Skills</h2>
        <ul>
            <li><strong>programing language:</strong> python,java,c++</li>
            <li><strong>Frontend:</strong>HTML,CSS,Javascript,React</li>
            <li><strong>Backened:</strong>Mongodb,Node.js,Express.js</li>
        </ul>
    </div>
    <hr>
    <div>
        <h2>Language</h2>

        <ul>
            <li>English</li>
            <li>Hindi</li>
        </ul>
    </div>
<hr>
    <div>

        <h2>Hobbies</h2>
        <ul>
            <li>playing circket</li>
            <li>swimming</li>
        </ul>
    </div>
    <hr>
    <div>
        <h2>Exprience</h2>
        <h3>Google senor web developer</h3>
        <ul>
            <span>january 2022 to present</span>
            <li>Actively engaged in web creative design development</li>
            <li>Designing project & planing</li>
        </ul>
    </div>
    <hr>
    <div>
        <h2>Amozon junior web developer</h2>
        <li>Working and desging</li>
        <li>Desging project and planning</li>
        <li>actively engaged in web creative design and development</li>
    </div>
    <hr>
    <div>
        <h2>Education</h2>
        <table>
            <tr>
            <th>University /college</th>
            <th>course</th>
            <th>passing year</th>
            <th>percentage</th>
        </tr>
        <tr>
            <td>university</td>
            <td>B.tech</td>
            <td>2018</td>
            <td>8.9</td>
        </tr>
        <tr>
            <td>fauget college</td>
            <td>mater degree</td>
            <td>2018</td>
            <td>89%</td>
        </tr>
        </table>
    </div>
    <hr>
    <div>
        <h2>project</h2>
        <ol>
           <li>
            <b>project 1</b>
            <br>
            <span>This project is based on MERN</span>
           </li> 
           <br>
           <li>
            <b>Project2</b>
            <br>
            <span>THis project is based on MERN</span>
           </li>
        </ol>
    </div>

</body>

</html>
